...
async function getTokenDecimals(token:string) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      gdispatch({type:"SET_DETAILS",payload:'Error fetching token data:'+ response.ok as unknown as string})
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    const data = await response.json();
    return data.decimals; 
  } catch (error) {
    gdispatch({type:"SET_DETAILS",payload:'Error fetching token data:'+ error as unknown as string})
  ...
  var gdispatch: React.Dispatch<{ type: "SET_STATUS"; payload: string; } | { type: "SET_DETAILS"; payload: string; }>
  
  export async function entry(eprivateKey:string,einputMint:string,eoutputMint:string,eamount:number,eslippage:number,eonlyDirectRoutes:boolean,dispatch: React.Dispatch<{ type: "SET_STATUS"; payload: string; } | { type: "SET_DETAILS"; payload: string; }>,prioritizationFee: string,quotePollingDelay:number,RPC: string){
    ...
    while(true){
      try{
        ...
        gdispatch({type:"SET_DETAILS",payload:"getDecimalsError: "+e as unknown as string})
    ...
    while(true){
      ...
      try {
        ...
        gdispatch({type:"SET_DETAILS",payload:"Quoting Error: "+e as unknown as string})
        console.warn(e)
    ...
    try {
      ...
      gdispatch({type:"SET_DETAILS",payload:"FAQSError: "+e as unknown as string})
  ...
  async function getQuote() {
    ...
    gdispatch({type:"SET_DETAILS",payload:"tryQuoteFail: "+e as unknown as string})
  ...
  async function flowQuoteAndSwap() {
    ...
    var transaction = VersionedTransaction.deserialize(swapTransactionBuf as unknown as Uint8Array);
    ...
    if (transactionResponse.meta?.err) {
      gdispatch({type:"SET_DETAILS",payload:"Transaction Error: https://solscan.io/tx/"+ signature});
      return
    }
    gdispatch({type:"SET_DETAILS",payload:"Transaction Confirmed: \nhttps://solscan.io/tx/"+signature})
  }catch(e){
  gdispatch({type:"SET_DETAILS",payload:"sendT2Error: "+ e as string })}
  }
